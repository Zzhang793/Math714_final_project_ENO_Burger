...
# Upwind, but also have entropy fix for the transonic rarefaction case
# So this is a basic low order method, but able to handle all cases
def upwind_finite_volume_solver(Nx,NT,dt):
    # Grid Spacing
...
# Input a is the left constant vlaue, b is the right constant value
# Output is the Godunov flux
def Gudunov_flux(a,b):
    if a<=b:
        if a<=0 and b>=0:
            return 0
        elif a<=0 and b<=0:
...
# ENO reconstruction use 3 stencils points for each polynomial
def ENO_interpolation_3rd_order(pre_U,Nx,h,k):
    if k!=3:
        raise Warning("This ENO works for k=3")
    # Define the helper vector with two ends extended as a result of the periodicity.
    UU = np.zeros(Nx + 2 * k)
    UU[k:Nx + k] = pre_U[:]
...
# High-order ENO finite volume method Euler Forward in time method
def ENO_finite_volume_solver_higher_order_ENO(Nx,NT,dt):
    ...
    for tt in range(NT):
        ...
        # Compute the Godunov flux, then use flux to update each cell average
        flux = np.zeros(Nx)
        for i in range(Nx):
            if i==0:
                a =pre_U[Nx-1]
...
# Using the optimal TVD Runge Kutta method for marching forward in time
def ENO_finite_volume_solver_higher_order_ENO_2nd_Runge_Kutta(Nx,NT,dt):  # trying new things
    ...
    for tt in range(NT):
        ...
        # Compute the Gudunov flux, then use flux to update each cell average
        flux = np.zeros(Nx)
        for i in range(Nx):
            if i==0:
                a =pre_U[Nx-1]
...
# The following setup is used in my report, to demonstrate that ENO is 5th order in space
level_ref = 14
...
# we will save the different solutions in here
U = []
# Save the N list
NN = []
...
for level in range(start_level,start_level+num_levels,1):
    ...
    dt = dt_ref   # Using the same time step
...
# plot the error as function of Ns
plt.figure(2)
...
# Plot all the solutions together in the same plot
# plt.figure(3)
# for level, (u_h, Nx) in enumerate(zip(U, NN)):
...
##########################################################################################
# 2nd part, show the solution at time t=0.26, after shock happens

# The report is using the following setup
level_ref = 8
Nx_ref = 2**level_ref   # 2**10 = 1024, 2**14 = 4096
T =  0.26
dt_ref = 0.0001
